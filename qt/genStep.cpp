...
int generateMarkovSteps(std::string pathToSMFile, double lengthOfSong, double bpm) {
	...
	*/
	...
	std::replace( prevPatt.begin(), prevPatt.end(), '3', '1');
	std::replace( prevPatt.begin(), prevPatt.end(), '2', '1');
	file << prevPatt;
	int count = 0;
	int fin = rand() % 3;
	int end = 0;
	...
	while (i < measures/4 || !finished) {
		...
		while (patternChosen == zeroPatt || patternChosen == "") {
		...
		file << patternChosen;
		std::cout << "chosen: " << patternChosen << std::endl;
		std::cout << "new prev: " << prevPatt << std::endl;
