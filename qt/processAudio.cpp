...
int process(std::vector<std::string> argv)
{
    ...
    std::ifstream ifs(argv.at(0), std::ifstream::binary);
	if (!(ifs && ifs.is_open())) {
		std::cout << "File does not exist, exiting..." << std::endl;
		exit(0);
	}

    std::cout << "File found: " << argv.at(0) << std::endl;
    ...
    int sr = chunk1Header->sampleRate;
    double bpm = getBPMDWT(channels, sr);
